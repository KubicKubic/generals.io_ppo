...
def train(cfg: TrainConfig, device=None):
    ...
    if cfg.resume_path is not None:
        ck = load_checkpoint(cfg.resume_path, policy, optimizer, device=device, opponent_pool_max=int(cfg.opponent.opponent_pool_max))
    ...
    for upd in range(start_update, int(cfg.total_updates) + 1):
        ...
        for step in range(int(cfg.rollout_len)):
            ...
            a1 = choose_opponent_action(env, opp, h1, device=device, random_prob=float(cfg.opponent.random_opp_prob), T=int(cfg.T))
            ...
            buf.add(
                ...
                a=a0.cpu(), logp=logp0.cpu(), v=v0.cpu(),
                r=r, done=done,
            )

            maybe_visualize_rollout_step(vs, env, upd, step, a0=int(a0.item()), a1=int(a1), r=float(r),
                                         v=float(v0.item()), logp=float(logp0.item()), done=done)
        ...
        if upd % int(cfg.save_every) == 0 or upd == int(cfg.total_updates):
            ...
            save_checkpoint(ckpt_path, policy, optimizer, update=upd, ep_count=ep_count, win_count=win_count,
                            opponent_pool=opponent_pool, rng_state=rng_state, config=config_for_ckpt)
            ...
            save_checkpoint(latest_path, policy, optimizer, update=upd, ep_count=ep_count, win_count=win_count,
                            opponent_pool=opponent_pool, rng_state=rng_state, config=config_for_ckpt)
